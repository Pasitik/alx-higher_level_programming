...
class Base():
    ...
    @staticmethod
    def raiseNotIntegerError(attribute):
        raise TypeError(f"{attribute} must be an integer")

    @staticmethod
    def raiseValueLessThanOrEqualToZeroError(attribute):
        raise ValueError(f"{attribute} must be > 0")

    @staticmethod
    def raiseValueLessThanZeroError(attribute):
        raise ValueError(f"{attribute} must be >= 0")
    ...
    def save_to_file(cls, list_objs):
        ...
        with open(f"{cls.__name__}.json", 'w') as f:
            if list_objs is None:
                ...
                string = cls.to_json_string(content)
